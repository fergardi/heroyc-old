<template lang="pug">
  #Mine
    ...
    .row
      ...
      .col-xs-4
        .panel.text-center.animated(v-bind:class='["panel-" + mine.Resource.rarity, { tada: states.monster.loot }, { hidden: !states.monster.loot }]')
          .panel-heading
            .panel-title
              i.fa.fa-fw.fa-lg(v-bind:class='"fa-" + mine.Resource.icon')  
              span {{mine.Resource.name}}
          .panel-body
            img.thumbnail.resource(v-bind:src='"dist/img/resources/" + mine.Resource.image + ".png"', v-bind:class='"panel-" + mine.Resource.rarity', data-toggle='tooltip', v-bind:title='mine.Resource.name')
            p {{mine.Resource.description}}
      .col-xs-4
        .panel.text-center.animated(v-bind:class='[{ flash: states.monster.melee }, { bounce: states.monster.distance }, { shake: states.monster.magic }, { zoomOut: states.monster.dead }, "panel-" + mine.Monster.type]')
          .panel-heading
            .panel-title
              i.ra.ra-fw.ra-lg(v-bind:class='"ra-" + mine.Monster.icon ')
              span {{mine.Monster.name}}
          .panel-body
            img.thumbnail.slot(v-bind:src='"dist/img/monsters/" + mine.Monster.image + ".png"', data-toggle='tooltip', v-bind:title='mine.Monster.name')
            hr
            .progress
              .progress-bar.progress-bar-danger(v-bind:style='"width: " + mine.Monster.vitality + "%"')
            hr
            a.list-group-item(v-for='spell in mine.Monster.Spells', v-bind:class='["list-group-item-" + spell.family]')
              img.icon(v-bind:src='"dist/img/spells/" + spell.type + "/" + spell.image + ".png"')
              span {{spell.name}}
              span.label.label-danger {{spell.damage}}
              span.label.label-primary {{spell.mana}}
...
<script>
  ...
  export default {
    ...
    created: function() {
      ...
      factory.getLocation(this.$route.params.mineId || 1, (data) => {
        self.mine = data;
        notification.danger('A wild <strong>' + self.mine.Monster.name + '</strong> appeared');
      });
    },
    watch: {
      'mine.Monster.vitality': function(value) {
        if(value <= 0) {
          self.states.buttons = true;
          setTimeout(function() {
            self.states.monster.dead = true;
            notification.success('The <strong>' + self.mine.Monster.name + '</strong> has been defeated');
          }, constants.notification.duration);
          setTimeout(function() {
            self.states.monster.loot = true;
            factory.addResource(self.player.id, self.mine.Resource.id, 1);
            notification.success('You looted <strong>' + self.mine.Resource.name + '</strong>');
    ...
    methods: {
      melee: function(dmg) {
        if (!self.states.buttons) {
          ...
          notification.danger('You caused <strong>-' + dmg + ' Vit</strong> to the <strong>' + self.mine.Monster.name + '</strong>');
          setTimeout(function(){
            self.states.buttons = false;
            self.states.monster.melee = false;
            self.mine.Monster.vitality -= dmg;
          }, constants.notification.duration);
        }
      },
      ...
      magic: function(name, dmg, heal, mana) {
        if (!self.states.buttons) {
          ...
          dmg === 0 ? notification.success('You healed <strong>+' + heal + ' Vit</strong>') : notification.info('You caused <strong>-' + dmg + ' Vit</strong> to the ' + self.mine.Monster.name);
          setTimeout(function(){
            self.states.buttons = false;
            dmg === 0 ? self.states.player.magic = false : self.states.monster.magic = false;
            dmg === 0 ? self.player.vitality += heal : self.mine.Monster.vitality -= dmg;
